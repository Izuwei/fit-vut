...
function test_int(){
    ...
    foreach ($src_files as $src){
        ...
        exec("python3.6 \"$interpret\" --source=\"$src\" --input=\"$in\" > \"$tmp\" 2>/dev/null", $com, $ret);
...
function test_both(){
    ...
    foreach ($src_files as $src){
        ...
        if (!is_writable($tmp = create_file(str_replace(".src", ".tmp", $src), "")))
            error("Nelze zapisovat do docasneho souboru $tmp.", 12);
        if (!is_writable($tmp2 = create_file(str_replace(".src", ".tmp2", $src), "")))
            error("Nelze zapisovat do docasneho souboru $tmp2.", 12);
        $out = create_file(str_replace(".src", ".out", $src), "");
        $in = create_file(str_replace(".src", ".in", $src), "");
        $rc = create_file(str_replace(".src", ".rc", $src), "0");

        $ret_ok = intval(trim(file_get_contents($rc)));  // Ref. navratova hodnota
        exec("php7.3 \"$parse\" < \"$src\" > \"$tmp\" 2>/dev/null", $com, $ret);

        if ($ret === 0){
            exec("python3.6 \"$interpret\" --source=\"$tmp\" --input=\"$in\" > \"$tmp2\" 2>/dev/null", $com, $ret);
            check_result($rc, $ret, $src, $out, $tmp2);  // Kontrola vysledku
        }
        elseif ($ret === $ret_ok){  // Ocekavana chyba v parseru
            $passed++;
            $html->add_result("green", $src, "OK");
        } else {    // Neocekavana chyba v parseru
            $failed++;
            $html->add_result("red", $src, "Unexpected return value $ret from Parse.php, expected $ret_ok");
        }
        exec("rm -f \"$tmp\" \"$tmp2\"");
    }
}
