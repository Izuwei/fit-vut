                       ...
                       const int        N)
{
  ...
  // Reduction loop over particles
  #pragma acc parallel loop reduction(+:com_x, com_y, com_z, com_w), present(p)
  for(int i = 0; i < N; i++)
  {
    com_x += p.positions_x[i] * p.weights[i];
    com_y += p.positions_y[i] * p.weights[i];
    com_z += p.positions_z[i] * p.weights[i];
    com_w += p.weights[i];
  }

  float4 com = { com_x / com_w, com_y / com_w, com_z / com_w, com_w };
  return com;
