...
struct Particles
{
  ...
  Particles(Particles *p) : N(p->N)
  {
    positions_x = new float[N];
    positions_y = new float[N];
    positions_z = new float[N];
    weights = new float[N];
    velocities_x = new float[N];
    velocities_y = new float[N];
    velocities_z = new float[N];

    std::memcpy(positions_x, p->positions_x, N * sizeof(float));
    std::memcpy(positions_y, p->positions_y, N * sizeof(float));
    std::memcpy(positions_z, p->positions_z, N * sizeof(float));
    std::memcpy(weights, p->weights, N * sizeof(float));
    std::memcpy(velocities_x, p->velocities_x, N * sizeof(float));
    std::memcpy(velocities_y, p->velocities_y, N * sizeof(float));
    std::memcpy(velocities_z, p->velocities_z, N * sizeof(float));
...
/**
 * Compute velocity
 * @param [in]  p_in     - Particles (input)
 * @param [in]  p_out    - Particles (output)
